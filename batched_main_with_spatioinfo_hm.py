...
if __name__ == "__main__":
    ...
    # N_relations=1 means treating the graph as homogeneous graph
    Globalmodel = GlobalTUL_withSpatio(args, n_relations=1)
    ...
    TrajTrajgraph_spatio = [torch.tensor(TrajTrajgraph_spatio[0], dtype=torch.long).to(device), torch.tensor(TrajTrajgraph_spatio[1], dtype=torch.float).to(device),
                            torch.tensor([0]*len(TrajTrajgraph_spatio[2]), dtype=torch.long).to(device)]
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            #     best_model = copy.deepcopy(Globalmodel.cpu())
    ...
    savepath = Path("./results_HM/") / "Spatio_{}_{}_spfactor_{}.txt".format(datapath.parts[2], datapath.parts[4], args.spatio)
    # savepath = Path("./results/") / "Spatio_{}_{}_spfactor_{}.txt".format(datapath.parts[2], datapath.parts[4], args.spatio)
    # savepath = Path("./robustness/") / "Spatio_{}_{}_spfactor_{}_m_{}.txt".format(datapath.parts[2], datapath.parts[4], args.spatio, args.m)
